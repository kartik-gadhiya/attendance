...
/**
 * Multiple Shift Segments Test Case
 * 
 * Testing break validation across 3 shift segments in a single day
 * Shift Configuration:
 * - Overall window: 7:00 AM - 11:00 PM
 * - Buffer: NONE (0 hours)
 * 
 * Shift Segments:
...
echo colorOutput("\n╔════════════════════════════════════════════════════════════════╗", 'blue') . "\n";
echo colorOutput("║     MULTIPLE SHIFT SEGMENTS TEST SUITE                         ║", 'blue') . "\n";
echo colorOutput("║     User: 3 | Date: 2026-01-01 | Buffer: NONE                  ║", 'blue') . "\n";
echo colorOutput("╚════════════════════════════════════════════════════════════════╝\n", 'blue') . "\n";

echo colorOutput("Shift Segments to be tested:", 'magenta') . "\n";
...
$gapTestsPassed = 0;
$gapTestsTotal = 10; // Phases 3 & 4 have 10 gap/boundary tests
...
if ($failCount === 0) {
    ...
    echo "\n" . colorOutput("⚠️  SOME TESTS FAILED. Please review:", 'red') . "\n";

    if ($failCount <= $gapTestsTotal) {
        echo colorOutput("⚠️  Gap validation may not be working correctly!", 'red') . "\n";
        echo colorOutput("⚠️  Breaks might be incorrectly allowed between shift segments!", 'red') . "\n";
    }
    echo "\n";
}
...
echo colorOutput("Expected structure:", 'cyan') . "\n";
echo colorOutput("  Segment 1: day_in(05:00) → breaks → day_out(10:00)", 'cyan') . "\n";
echo colorOutput("  GAP: 10:00 - 12:00 (NO BREAKS ALLOWED)", 'cyan') . "\n";
echo colorOutput("  Segment 2: day_in(12:00) → breaks → day_out(20:00)", 'cyan') . "\n";
echo colorOutput("  GAP: 20:00 - 21:00 (NO BREAKS ALLOWED)", 'cyan') . "\n";
echo colorOutput("  Segment 3: day_in(21:00) → breaks → day_out(02:00)", 'cyan') . "\n\n";
